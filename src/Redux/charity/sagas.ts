...
function* getDetailVolunteerAction({ payload }: BaseAction<string>) {
  try {
    const data = yield call(PartnerService.getDetailVolunteerAction, payload);
    yield put(
      actions.getDetailVolunteerAction.success({
        data,
      })
    );
  } catch (error: any) {
...
export default function* sagas() {
  yield all([
    ...
    takeLatest(GET_DETAIL_VOLUNTEER_ACTION.REQUEST, getDetailVolunteerAction),
  ]);
}
