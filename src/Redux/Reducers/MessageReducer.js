    ...
    switch (action.type) {
        ...
        case ActionType.GET_MORE_MESSAGE:
            ...
            return {
                ...
                currRoomChatting: {
                    ...state.currRoomChatting,
                    messages: [...state.currRoomChatting.messages, ...action.payload.data]
                } 
            }
        case ActionType.GET_NEWEST_MESSAGE:
            ...
            return {
                ...
                currRoomChatting: {  
                    ...state.currRoomChatting,
                    messages: [...action.payload.data, ...state.currRoomChatting.messages]
                }
            }
        ...
        case ActionType.UPDATE_LIST_LASTED_MESSAGES:
            ...
            return {
                ...state,
                listLastedMessages: [...state.listLastedMessages, ...action.payload.data]
            }
        ...
        case ActionType.GET_NEW_MESSAGE:
            ...
            let tempListLastedMessages = [...state.listLastedMessages].filter(itemFilter => itemFilter._id !== action.payload.data.conversationId)

            if (!isEmpty(state.currRoomChatting)) {
                if (state?.currRoomChatting?.infoCurrRoomChatting?._id == action.payload.data.conversationId) {
                    return {
                        ...state,
                        currRoomChatting: {
                            ...state.currRoomChatting,
                            messages: [action.payload.data.message, ...state.currRoomChatting.messages]
                        },
                        listLastedMessages: [action.payload.data.conversation, ...tempListLastedMessages]
                    }
                }
            }
            return {
        ...
        case ActionType.UPDATE_SPECIFIC_MESSAGE:
            let tempCurrRoomChattingForUpdateSpecific = { ...state.currRoomChatting }

            if (!isEmpty(tempCurrRoomChattingForUpdateSpecific?.infoCurrRoomChatting) && tempCurrRoomChattingForUpdateSpecific?.infoCurrRoomChatting?._id == action?.payload?.data?.message?.conversationId) {
                // tempCurrChatting = { ...state.currChatting } 
                let findIndexItemMessageHasSeen = tempCurrRoomChattingForUpdateSpecific.messages.findIndex(itemFind => itemFind._id == action.payload.data.message?._id);
                if (findIndexItemMessageHasSeen !== -1) {
                    tempCurrRoomChattingForUpdateSpecific.messages[findIndexItemMessageHasSeen] = action.payload.data.message
                }
            }
        ...
        case ActionType.UPDATE_VIEWER_MESSAGE:
            console.log({ xyz: action.payload.data });
            let tempListLastedMessagesUpdateViewer = [...state.listLastedMessages];
            let tempCurrRoomChatting = { ...state.currRoomChatting }

            if (!isEmpty(tempCurrRoomChatting?.infoCurrRoomChatting) && tempCurrRoomChatting?.infoCurrRoomChatting?._id == action?.payload?.data?.conversationId) {
                // tempCurrChatting = { ...state.currChatting } 
                let findIndexItemMessageHasSeen = tempCurrRoomChatting.messages.findIndex(itemFind => itemFind._id == action.payload.data.data.messages[0]?._id);
                if (findIndexItemMessageHasSeen !== -1) {
                    tempCurrRoomChatting.messages[findIndexItemMessageHasSeen] = action.payload.data.data.messages[0]
                }
            }


            let finding = tempListLastedMessagesUpdateViewer.findIndex(itemFind => itemFind?._id == action?.payload?.data?.conversationId);

            if (finding !== -1) {

                tempListLastedMessagesUpdateViewer[finding].latestMessage = action.payload.data.data.messages[0];
            }
