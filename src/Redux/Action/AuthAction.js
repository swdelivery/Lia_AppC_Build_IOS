...
const _checkInfoDevice = async () => {
    ...
    console.log({ nameDevice, platformDevice, versionPlatfromDevice, macAddressDevice });
}

export const firebaseResetPassword = (data) => {
    return Axios.post(`${URL_FOR_PARTNER}/firebase-id-token/reset-password`, data)
        .then(res => {
            console.log(useLogStyle + '----FETCHING SUCCESS: ', setLogStyle('green'), { firebaseResetPassword: res });
            _checkSuccess(res)
            return res
        })
        .catch(err => {
            console.log(useLogStyle + '----FETCHING FAIL: ', setLogStyle('red'), { firebaseResetPassword: err });
            _checkError(err)
            return err
            // _checkError(err) 
        })
...
export const partnerAccountLogout = (data) => {
    return Axios.post(`${URL_FOR_PARTNER}/partner-account/logout`, data)
        .then(res => {
            console.log(useLogStyle + '----FETCHING SUCCESS: ', setLogStyle('green'), { partnerAccountLogout: res });
            _checkSuccess(res)
            return res
        })
        .catch(err => {
            console.log(useLogStyle + '----FETCHING FAIL: ', setLogStyle('red'), { partnerAccountLogout: err });
            _checkError(err)
            return err
            // _checkError(err) 
        })
...
export const partnerAccountRegister = (data) => {
    return Axios.post(`${URL_FOR_PARTNER}/partner-account/register`, data)
        .then(res => {
            console.log(useLogStyle + '----FETCHING SUCCESS: ', setLogStyle('green'), { partnerAccountRegister: res });
            // _checkSuccess(res)
            return res
        })
        .catch(err => {
            console.log(useLogStyle + '----FETCHING FAIL: ', setLogStyle('red'), { partnerAccountRegister: err });
            _checkError(err)
            return err
            // _checkError(err) 
        })
...
export const forceVerifyAccount = (data) => {
    return Axios.post(`${URL_FOR_PARTNER}/partner-account/force-verify-account`, data)
        .then(res => {
            console.log(useLogStyle + '----FETCHING SUCCESS: ', setLogStyle('green'), { forceVerifyAccount: res });
            // _checkSuccess(res)
            return res
        })
        .catch(err => {
            console.log(useLogStyle + '----FETCHING FAIL: ', setLogStyle('red'), { forceVerifyAccount: err });
            _checkError(err)
            return err
            // _checkError(err) 
        })
...
export const verifyAccount = (data) => {
    return Axios.post(`${URL_FOR_PARTNER}/firebase-id-token/verify-account`, data)
        .then(res => {
            console.log(useLogStyle + '----FETCHING SUCCESS: ', setLogStyle('green'), { verifyAccount: res });
            // _checkSuccess(res)
            return res
        })
        .catch(err => {
            console.log(useLogStyle + '----FETCHING FAIL: ', setLogStyle('red'), { verifyAccount: err });
            _checkError(err)
            return err
            // _checkError(err) 
        })
...
const createNewTokenFcm = (data) => {
    return Axios.post(`${URL_FOR_PARTNER}/partner-device`, data)
        .then(res => {
            console.log(useLogStyle + '----FETCHING SUCCESS: ', setLogStyle('green'), { createNewTokenFcm: res });
        })
        .catch(err => {
            console.log(useLogStyle + '----FETCHING FAIL: ', setLogStyle('red'), { createNewTokenFcm: err });
            // _checkError(err) 
        })
}
...
export const checkRefreshToken = async () => {
    ...
    if (!tokenSTR) {
        Store.dispatch({ type: ActionType.CHECK_AUTH_PROCESSING, payload: { flag: true } })
    ...
    Axios.get(`${URL_FOR_PARTNER}/partners/profile`, {})
        .then(async res => {
            console.log(useLogStyle + '----FETCHING SUCCESS: ', setLogStyle('green'), { getProfile: res });
            let fcmTokenSTR = await AsyncStorage.getItem('fcmToken');
            console.log({ fcmTokenSTR });
            let findTokenFCM = res.data.data.devices.find(itemFind => itemFind.fcmToken == fcmTokenSTR);
            ...
            console.log({nameDevice,fcmTokenSTR,platformDevice,versionPlatfromDevice,macAddressDevice,versionApp});
            ...
            Store.dispatch({ type: ActionType.SAVE_INFO_USER, payload: { data: res.data.data } })
            SocketInstance.getInstance()
            Store.dispatch({ type: ActionType.LOGIN_SUCCESS, payload: { flag: true } })
            Store.dispatch({ type: ActionType.CHECK_AUTH_PROCESSING, payload: { flag: true } })
        })
        .catch(err => {
            console.log(useLogStyle + '----FETCHING FAIL: ', setLogStyle('red'), { getProfile: err });
            Store.dispatch({ type: ActionType.CHECK_AUTH_PROCESSING, payload: { flag: true } })
        })
}
...
export const login = (props) => {
    return async dispatch => {
        ...
        Axios.post(`${URL_FOR_PARTNER}/partner-account/login`, props)
            .then(async res => {
                console.log({ ...res });
                console.log(useLogStyle + '----FETCHING SUCCESS: ', setLogStyle('green'), { login: res });
                console.log('then');
            ...
            .catch(err => {
                dispatch({ type: ActionType.LOADING_DONE, payload: null })
                console.log(useLogStyle + '----FETCHING FAIL: ', setLogStyle('red'), { login: err });
                ...
                if (err?.response?.data?.message == 'Tài khoản chưa xác nhận') {
                    ...
                    return navigation.navigate(ScreenKey.ACTIVATION, { phoneNumber: newFormatPhone, fullPhone: props?.phone?.phoneNumber, password: props?.password })
                }
        ...
        Axios.post(`${URL_FOR_PARTNER}/partner-account/login`, props)
            ...
            .catch(err => {
                ...
                if (err?.response?.data?.message == 'Tài khoản chưa xác nhận') {
                    ...
                    console.log({ phoneNumber: newFormatPhone, fullPhone: props?.phone?.phoneNumber, password: props?.password, routeName: routeNameForGoback });
                    
                    return navigation.navigate(ScreenKey.ACTIVATION_IN_APP, { phoneNumber: newFormatPhone, fullPhone: props?.phone?.phoneNumber, password: props?.password, routeName: routeNameForGoback })
                }
                if (err?.message == 'Network Error') {
                    return Alert.alert(
                        "Lỗi",
...
export const register = (data) => {
    ...
    return Axios.post(`${URL_FOR_PARTNER}/partner-account/register`, data)
        .then(res => {
            console.log({ ...res });
            console.log(useLogStyle + '----FETCHING SUCCESS: ', setLogStyle('green'), { register: res });

            return res
        })
        .catch(err => {
            console.log(useLogStyle + '----FETCHING FAIL: ', setLogStyle('red'), { register: err });
            _checkError(err)
        })
}
