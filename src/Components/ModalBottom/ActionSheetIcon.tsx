...
const ActionSheetIcon = memo(() => {
  ...
  const { showActionSheetIcon: { flag: visible, data: options } } = useSelector(getStateActionSheetIcon)
  ...
  const Option = ({ data, lasted, onPress }) => {
    ...
    const _renderIcon = (type) => {
      switch (type) {
        case 'booking':
          return (
            <IconBooking width={8 * 3} height={8 * 3} />
          )
        case 'faceAI':
          return (
            <IconMirrorr width={8 * 3} height={8 * 3} />
          )
        default:
          break;
      }
