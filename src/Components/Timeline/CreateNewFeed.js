...
const CreateNewFeed = props => {
    ...
    const _handleConfirmCreateNewFeed = async () => {
        ...
        if (!isEmpty(galleryCurr)) {
            resultUploadImagePost = await handleApi(_uploadModule({
                moduleName: 'post',
                files: galleryCurr
            }))
        ...
        if (!isEmpty(listVideoForUpload)) {
            resultUploadVideoPost = await handleApi(_uploadModule({
                moduleName: 'post',
        ...
        if (!isEmpty(listDocumentsForUpload)) {
            resultUploadDocumentPost = await handleApi(_uploadModuleDocument({
                moduleName: 'post',
        ...
        dispatch(confirmCreateNewPost({
            title: '.',
            content: contentNewFeedCreate.trim(),
            images: !isEmpty(galleryCurr) ? resultUploadImagePost.data.map(item => item._id) : [],
            videos: !isEmpty(listVideoForUpload) ? resultUploadVideoPost.data.map(item => item._id) : [],
            documents:!isEmpty(listDocumentsForUpload) ? resultUploadDocumentPost.data.map(item => item._id) : []
        }))
    ...
    return (
        <View style={[styles.container]}>
            ...
            <View style={[styles.content__header, styleElement.rowAliCenter, shadow, { zIndex: 1 }]} key="0">
                ...
                style={{ padding: _moderateScale(8), width: _moderateScale(8 * 8) }} onPress={() => navigation.goBack()}>
                    <Image
                        style={sizeIcon.lg}
                        source={require('../../Icon/cancel.png')} />
                </TouchableOpacity>

                <Text style={[stylesFont.fontNolan500, { fontSize: _moderateScale(16), color: Color.GREY_FOR_TITLE }]}>Tạo bài viết</Text>
                ...
                <TouchableOpacity
                    ...
                    style={{ padding: _moderateScale(8), width: _moderateScale(8 * 8), alignItems: 'flex-end' }}>
            ...
            <ScrollView scrollIndicatorInsets={{right:1}} contentContainerStyle={{ flex: 1 }} style={styles.inputView}>
                    ...
                    <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                        <View style={[styleElement.rowAliCenter]}>
                            <FastImage
                                ...
                                uri={
                                    infoUserRedux?.infoUser?.profile?.fileAvatar?.link ? `${URL_ORIGINAL}${infoUserRedux?.infoUser?.profile?.fileAvatar?.link}` : URL_AVATAR_DEFAULT
                                }
                ...
                <View style={{
                    ...
                    {
                        !isEmpty(listVideoForUpload) && listVideoForUpload.map((itemMap, indexMap) => {
                            console.log();
                    ...
                    {
                        !isEmpty(listDocumentsForUpload) && listDocumentsForUpload.map((itemMap, indexMap) => {
                            return (
                                <TouchableOpacity
                                    key={indexMap}
                                    style={[styleElement.rowAliCenter, styles.btnVideoWatch]}
