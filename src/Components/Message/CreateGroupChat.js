...
import { Image, ImageBackground, Platform, StyleSheet, Text, TouchableOpacity, UIManager, View ,FlatList, ScrollView, TextInput } from 'react-native';
...
const CreateGroupChat = props => {
    ...
    const [listMembersOfSystemLocalComp, setListMembersOfSystemLocalComp] = useState(membersRedux?.listMembersOfSystem)
    ...
    const _renderItemMember = ({ item, index }) => {
        return (
            <View style={[styleElement.rowAliCenter, { flex: 1 }, index == listMembersOfSystemLocalComp.length - 1 && { marginBottom: _moderateScale(8 * 6) }]}>
                <Image
                    style={{
                ...
                {
                    currListMembersInGroupChat.find(itemFind => itemFind.userId == item._id) ?
                        <TouchableOpacity
    ...
    return (
        <View style={{ flex: 1, backgroundColor: Color.WHITE }}>
            <View style={[styles.content__header, styleElement.rowAliCenter, shadow, { justifyContent: 'space-between' }]} key="0">
                <TouchableOpacity
                    onPress={() => {
                        navigation.goBack()
                    }}
            ...
            <Collapsible collapsed={headerCollapsed}>
                <View style={[styleElement.rowAliCenter, { marginTop: _moderateScale(8) }]}>
                    <TouchableOpacity style={[styles.btnCamera, { marginLeft: _moderateScale(16) }]}>
                        <Image style={sizeIcon.llg} source={require('../../Icon/camera_blue.png')} />
                    </TouchableOpacity>
                    <TextInput
                        value={titleGroupChat}
                        onChangeText={(content) => _onChangeTitleGroupChat(content)}
                        placeholderTextColor={Color.GREY}
                        style={[stylesFont.fontNolan500, { fontSize: _moderateScale(18), flex: 1, marginHorizontal: _moderateScale(14) }]}
                        placeholder="Đặt tên nhóm" />
                </View>
            </Collapsible>

            <View style={{ width: _width, paddingHorizontal: _moderateScale(14), marginVertical: _moderateScale(8) }}>
                <View style={[styleElement.rowAliCenter, { width: "100%", backgroundColor: Color.BG_GREY_OPACITY, borderRadius: _moderateScale(8) }]}>
                    ...
                    <TextInput
                        ...
                        style={[{ marginVertical: _heightScale(6), paddingVertical: _moderateScale(0), fontSize: _moderateScale(14), flex: 1, paddingRight: _moderateScale(16) }, Platform.OS == 'ios' && { paddingVertical: _moderateScale(8) }]} />
                </View>
            </View>
            ...
            <View>
                <ScrollView
                    ...
                    horizontal>
                    {
                        !isEmpty(currListMembersInGroupChat) && currListMembersInGroupChat.map((item, index) => {
                            ...
                            return (
                                <TouchableOpacity
                                    ...
                                    <ImageBackground
                                        ...
                                        source={{ uri: item?.fileAvatar?.link ? `${URL_ORIGINAL}${item?.fileAvatar?.link}` : URL_AVATAR_DEFAULT }}
                                    >
                                        <View style={{
                                            position: 'absolute',
            ...
            <View style={{ flex: 1 }}>
                {
                    !isEmpty(listMembersAfterFilter) ?
                        <FlatList
                            data={!isEmpty(listMembersAfterFilter) ? listMembersAfterFilter : []}
                        ...
                        <>
                            {
                                !isEmpty(listMembersOfSystemLocalComp) ?
                                    <FlatList
                                        data={!isEmpty(listMembersOfSystemLocalComp) ? listMembersOfSystemLocalComp : []}
                                        renderItem={_renderItemMember}
                                        keyExtractor={_awesomeChildListKeyExtractor}
                                    />
                                    :
