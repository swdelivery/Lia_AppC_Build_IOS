...
import { BASE_COLOR, BG_GREY_OPACITY_5, BLACK, BORDER_COLOR, GREY, RED, WHITE } from '@Constant/Color'
...
const MultiInput = ({ title, require, number, icon, setError, error, setValue, value }: Props) => {
  ...
  return (
    <TouchableWithoutFeedback onPress={() => {
      ...
      <View
        style={[styles.container, onFocused && { borderWidth: 1, borderColor: BASE_COLOR }, error && { borderColor: RED }]}>
        ...
        <Row paddingLeft={8 * 6} gap={8}>
          ...
          <View>
            <TextInput
              ...
              maxLength={2}
              style={[styles.input, { opacity: onFocused || valueTextInputDate?.length > 0 ? 1 : 0 }]} />
            <View style={[
              styles.input__underLine,
              { opacity: onFocused || valueTextInputDate?.length > 0 ? 1 : 0 }
            ]} />
          </View>
          <Text style={{ opacity: onFocused || valueTextInputDate?.length > 0 ? 1 : 0 }}>/</Text>
          ...
          <View>
            <TextInput
              ...
              keyboardType={number ? "number-pad" : 'default'}
              maxLength={2}
              style={[styles.input, { opacity: onFocused || valueTextInputMonth?.length > 0 ? 1 : 0 }]} />
            <View style={[
              styles.input__underLine,
              { opacity: onFocused || valueTextInputDate?.length > 0 ? 1 : 0 }
            ]} />
