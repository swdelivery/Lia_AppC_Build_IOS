...
const FeedBackBranch = memo((props) => {
    ...
    useEffect(() => {
        if (props?.route?.params?.currentBranch?._id) {
            _getReview()
        }
    }, [props?.route?.params?.currentBranch])


    const _getReview = async () => {
        var review = await getBranchReviewByCode(props?.route?.params?.currentBranch?.code)
    ...
    const _getReviewMore = async (_idMore) => {
        setIsLoadMore(true)
        var review = await getBranchReviewByCode(props?.route?.params?.currentBranch?.code, _idMore)
