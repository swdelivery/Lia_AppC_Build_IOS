import { Alert, Image, Platform, StyleSheet, TouchableOpacity, TouchableWithoutFeedback, View, _Image } from 'react-native'
...
import { IconCommentWhite, IconHeartFilled, IconHeartWhite, IconPlayWhite } from '@Components/Icon/Icon';
...
const VideoPlayer = memo((props) => {
    ...
    useEffect(() => {
        if (currVideoData?.duration && isPlaying) {
            ...
            tranDotX.value = withTiming(WIDTH_PROCESS_BAR, {
                duration: currVideoData.duration * 1000 - currPlayingTimeRef.current * 1000
            })
        } else {
    ...
    const eventHandler = useAnimatedGestureHandler({
        ...
        onEnd: (event, ctx) => {
            ...
            let timeSeek = event.absoluteX * currVideoData?.duration / WIDTH_PROCESS_BAR;
    ...
    const progressBarFillAnim = useAnimatedStyle(() => {
        ...
        const interpolateFillProgressBar = interpolate(tranDotX.value,
            [0, WIDTH_PROCESS_BAR],
            [0, WIDTH_PROCESS_BAR]
        )
    ...
    return (
        <View style={{
            ...
            <TouchableOpacity
                ...
                style={{
                    position: 'absolute', top: 0, left: 0, bottom: 0, right: 0, zIndex: 2, justifyContent: 'center', alignItems: 'center'
                }}>
            ...
            <View style={{ width: _width, height: _heightScale(8 * 8), position: 'absolute', zIndex: 4, bottom: 0, backgroundColor: BLACK }}>
    ...
    useEffect(() => {
        if (props?.startHeartedAnimation == 'doing') {
            ...
            opacityHearted.value = withTiming(1, { duration: 200 }, (fns) => {
                if (fns) {
                    animationHeartedProgress.value = withTiming(1, { duration: 1000 }, (fns) => {
                        if (fns) {
                            opacityHearted.value = withTiming(0, { duration: 500 }, (fns) => {
        ...
        <>
            {
                props?.startHeartedAnimation == 'doing' ?
                    <AnimatedLottieView
                        style={[{ width: 300, height: 300 }, animOpacityHearted, animTranYHeart]}
