...
const RightEyeResult = memo((props) => {
    ...
    useEffect(() => {
        if (props?.startRightResult) {
            tranYBox.value = withTiming(HEIGHT_BOX_CHOICE * (listEyeResult.length - 1), { duration: 1000 }, (isFinished) => {
                if (isFinished) {
                    tranYBox.value = withTiming(HEIGHT_BOX_CHOICE * (listEyeResult?.length - 7), { duration: 500 }, (isFinished) => {
                        if (isFinished) {
                            scaleEyeChoiced.value = withTiming(1.15, { duration: 500 })


                            numberValue.value = withTiming(97, { duration: 1000 })
                            ...
                            <Animated.View style={[styles.animatedChoiceBox, animBoxChoice]}>
                                ...
                                style={{
                                    width: _widthScale(8 * 2),
                                        ...
                                        position:'absolute',
                                        left:-_widthScale(8 * 2),
                                        top:_widthScale(8*4)- _widthScale(8),
                                        resizeMode:'contain'
                                }}
                                 source={require('../../../Image/arrowLeftBoxChoice.png')} 
                                />
                                {/* <Image
                                    style={{
                                ...
                                <Animated.View style={[{
                                    ...
                                    source={require('../../../Image/leftBoxChoice.png')}
                                    />
...
const styles = StyleSheet.create({
    ...
    rightEyeResult: {
        ...
        position: 'absolute',
        zIndex: 10,
        borderTopLeftRadius: _moderateScale(8 * 3),
        borderBottomLeftRadius: _moderateScale(8 * 3),
        alignItems: 'center',
        justifyContent: 'center'
    }
})