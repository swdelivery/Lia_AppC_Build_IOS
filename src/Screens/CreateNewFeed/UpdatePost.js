...
import { updatePartnerDiaryv2 } from "../../Redux/Action/PartnerDiary";
...
import { getPostByIdv2, updatePostv2 } from "../../Redux/Action/PostAction";
...
const UpdatePost = (props) => {
  ...
  const [routes] = useState([{ key: "second", title: "Hình ảnh" }]);
        ...
        setCurrPartnerDiary((old) => {
          return {
            ...
            imageBeforeTreatment: [
              ...old?.imageBeforeTreatment,
              ...resultUploadImageBefore?.data?.data,
            ],
          };
        });
      })
  ...
  const pickImageDailyDiary = async (dailyDiary) => {
    ...
    ImagePicker.openPicker({
      ...
      .then(async (images) => {
        ...
        let findIndex = tempCurrPartnerDiary?.dailyDiaryArr?.findIndex(
          (item) => item?._id == dailyDiary?._id
        );
        if (findIndex !== -1) {
  ...
  const _handleDeleteImageBefore = async (_id) => {
    if (currPartnerDiary?.imageBeforeTreatment?.length == 1) {
      return alertCustomNotAction(
        `Thông báo`,
        `Ảnh trước điều trị phải chứa ít nhất 1 tấm`
      );
    }
    ...
    setCurrPartnerDiary((old) => {
      return {
        ...old,
        imageBeforeTreatment: currPartnerDiary?.imageBeforeTreatment?.filter(
          (item) => item?._id !== _id
        ),
      };
    });
  ...
  const _handleDeleteImageAfter = async (_id) => {
    if (currPartnerDiary?.imageAfterTreatment?.length == 1) {
      return alertCustomNotAction(
        `Thông báo`,
        `Ảnh sau điều trị phải chứa ít nhất 1 tấm`
      );
    }
    ...
    setCurrPartnerDiary((old) => {
      return {
        ...old,
        imageAfterTreatment: currPartnerDiary?.imageAfterTreatment?.filter(
          (item) => item?._id !== _id
        ),
      };
    });
  ...
  const _handleConfirmEditDescription = async (
    _idDailyDiary,
    newDescription
  ) => {
    ...
    let findIndex = tempCurrPartnerDiary?.dailyDiaryArr?.findIndex(
      (item) => item?._id == _idDailyDiary
    );
    if (findIndex !== -1) {
      tempCurrPartnerDiary.dailyDiaryArr[findIndex].description =
        newDescription;
    }
  ...
  const _handleDeleteImageDailyDiary = async (itemParent, itemChild) => {
    ...
    itemParent = {
      ...itemParent,
      images: itemParent?.images?.filter(
        (item) => item?._id !== itemChild?._id
      ),
    };
  ...
  const _handleUpdatePost = async () => {
    ...
    if (resultUpdatePost?.isAxiosError)
      return dispatch({
    ...
    tempCurrPartnerDiary.dailyDiaryArr = currPartnerDiary?.dailyDiaryArr?.map(
      (item) => {
        return {
          ...item,
    ...
    let resultUpdatePartnerDiary = await updatePartnerDiaryv2(
      currPartnerDiary?._id,
      {
        imageBeforeTreatment: currPartnerDiary?.imageBeforeTreatment?.map(
          (item) => item?._id
        ),
        imageAfterTreatment: currPartnerDiary?.imageAfterTreatment?.map(
          (item) => item?._id
        ),
        dailyDiaryArr: tempCurrPartnerDiary?.dailyDiaryArr,
      }
    );
    if (resultUpdatePartnerDiary?.isAxiosError)
      return dispatch({
        type: ActionType.LOADING_DONE,
        payload: null,
      });
        ...
        return (
          <ScrollView>
                ...
                >
                  {currScope?.slug == "PUBLIC" ? (
                    <>
                      <Image
                        style={sizeIcon.xs}
                        source={require("../../NewIcon/earthGrey.png")}
                      />
                      <Text
                        style={{
                          ...stylesFont.fontNolanBold,
                      ...
                      <Image
                        style={sizeIcon.xxs}
                        source={require("../../NewIcon/downGrey.png")}
                      />
                    </>
                  ) : (
                    <></>
                  ...
                  {currScope?.slug == "PRIVATE" ? (
                    <>
                      <Image
                        style={sizeIcon.xs}
                        source={require("../../NewIcon/lockGrey.png")}
                      />
                      <Text
                        style={{
                          ...stylesFont.fontNolanBold,
            ...
            >
              ...
              <ScrollView showsHorizontalScrollIndicator={false} horizontal>
                    ...
                    >
                      <TouchableOpacity
                        onPress={() => {
                          ...
                          setListImagesSeeCurr(
                            currPartnerDiary?.imageBeforeTreatment
                          );
                        }}
                        style={{
                ...
                >
                  ...
                  >
                    <Image
                      style={[sizeIcon.lxlg, { opacity: 0.5 }]}
                    ...
                    >
                      <TouchableOpacity
                        onPress={() => {
                          ...
                          setIndexCurrImageView(index);
                          setListImagesSeeCurr(
                            currPartnerDiary?.imageAfterTreatment
                          );
                        }}
                        style={{
                          width: _moderateScale(8 * 11.8),
            ...
            >
              ...
              {currPartnerDiary?.dailyDiaryArr?.length > 0 &&
                currPartnerDiary?.dailyDiaryArr?.map((item, index) => {
                  return (
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text
          style={{ ...stylesFont.fontNolan500, fontSize: _moderateScale(16) }}
        >
          Chỉnh sửa
        </Text>
        <View style={{ width: _moderateScale(8 * 10), alignItems: "flex-end" }}>
          {index == "0" ? (
            <TouchableOpacity
              ...
              style={[
                styles.btnConfirm,
                { backgroundColor: BLUE_FB, width: _moderateScale(8 * 9) },
              ]}
            >
              <Text
                style={{
