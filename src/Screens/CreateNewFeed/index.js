...
const DiaryOfTreatment = (props) => {
  ...
  const [showModalCreateDesDailyDiary, setShowModalCreateDesDailyDiary] =
    useState({
      show: false,
      data: {},
    });
  ...
  const pickImageBefore = async () => {
    ImagePicker.openPicker({
      ...
      .then(async (images) => {
        ...
        let listIdImageHasUpload = resultUploadImageBefore?.data?.data?.map(
          (item) => item?._id
        );

        let result = await updatePartnerDiaryv2(currPartnerDiary?._id, {
          imageBeforeTreatment: [
            ...currPartnerDiary?.imageBeforeTreatment?.map((item) => item?._id),
            ...listIdImageHasUpload,
          ],
        });
  ...
  const pickImageAfter = async () => {
    ImagePicker.openPicker({
      ...
      .then(async (images) => {
        ...
        let result = await updatePartnerDiaryv2(currPartnerDiary?._id, {
          imageAfterTreatment: [
            ...currPartnerDiary?.imageAfterTreatment?.map((item) => item?._id),
            ...listIdImageHasUpload,
          ],
        });
        if (result?.isAxiosError) return;
    ...
    ImagePicker.openPicker({
      ...
      .then(async (images) => {
        ...
        let result = await updatePartnerDailyDiaryv2(dailyDiary?._id, {
          images: [
            ...dailyDiary?.images?.map((item) => item?._id),
            ...listIdImageHasUpload,
          ],
        });
        if (result?.isAxiosError) return;
  ...
  const _handleDeleteImageBefore = async (_id) => {
    ...
    let result = await updatePartnerDiaryv2(currPartnerDiary?._id, {
      imageBeforeTreatment: currPartnerDiary?.imageBeforeTreatment
  ...
  const _handleDeleteImageAfter = async (_id) => {
    ...
    let result = await updatePartnerDiaryv2(currPartnerDiary?._id, {
      imageAfterTreatment: currPartnerDiary?.imageAfterTreatment
        ?.filter((item) => item?._id !== _id)
        ?.map((item) => item?._id),
    });
    if (result?.isAxiosError) return;
  ...
  const _handleConfirmCreateDescription = async (
    _idDailyDiary,
    newDescription
  ) => {
    console.log({ _idDailyDiary, newDescription });
  ...
  const _handleDeleteDailyDiary = async (idDailyDiary) => {
    ...
    setCurrPartnerDiary((old) => {
      return {
        ...old,
        dailyDiaryArr: old.dailyDiaryArr?.filter(
          (item) => item?._id !== idDailyDiary
        ),
      };
    });
  };
  ...
  const renderScene = ({ route }) => {
    switch (route.key) {
      case "first":
        return <ListDiaryForCreatePost pickDiary={_pickDiary} />;
      case "second":
        return (
          <ScrollView>
            <View
            ...
            >
              ...
              {!currPartnerDiary?.dailyDiaryArr[0]?.created ||
              !moment(new Date()).isSame(
                moment(currPartnerDiary?.dailyDiaryArr[0]?.created),
                "day"
              ) ? (
                <>
