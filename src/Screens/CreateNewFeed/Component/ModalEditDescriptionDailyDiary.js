...
import { Platform, KeyboardAvoidingView, Text, StyleSheet, View, TextInput, ScrollView, Image, TouchableOpacity, Keyboard } from 'react-native';
...
const ModalEditDescriptionDailyDiary = memo((props) => {
    ...
    return (
        <Modal
            ...
            <KeyboardAvoidingView
                // behavior="position"
                keyboardVerticalOffset={Platform.OS == 'ios' && getBottomSpace() == 0 ? _heightScale(100) : _heightScale(100)}
                behavior={Platform.OS == 'ios' ? 'position' : null}
                enabled
            >

                <View style={styles.container}>
                    <ScrollView keyboardShouldPersistTaps={'handled'} style={{ flexGrow: 1, paddingVertical: _moderateScale(8 * 2) }}>
                        <View style={[{ paddingHorizontal: _moderateScale(8 * 2), justifyContent: 'space-between', flexDirection: 'row', alignItems: 'center' }]}>
                            ...
                            style={[styleElement.centerChild,{ width: _moderateScale(8 * 6), height: _moderateScale(8 * 3), backgroundColor: BLUE_FB , borderRadius:_moderateScale(4)}]}>
...
export default ModalEditDescriptionDailyDiary;