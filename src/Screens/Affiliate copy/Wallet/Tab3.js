...
const Tab3 = memo((props) => {
    ...
    const [listBookingReferral, setListBookingReferral] = useState([])
    ...
    useEffect(() => {
        ...
        _getBookingReferral(props?.currMonthFilter, props?.currLevelFilter)

        console.log({infoUserRedux});
    }, [props?.currMonthFilter, props?.currLevelFilter])

    const _getBookingReferral = async (currMonthFilter, currLevelFilter) => {
        setIsLoading(true)
        ...
        if (currLevelFilter) {
            if (currLevelFilter == 'all') {
                ...
                tempCondition['level'] = Number(currLevelFilter)
            }
        }
        let result = await getBookingReferral({
            condition: tempCondition
        })
        if (result?.isAxiosError) return setIsLoading(false)
        console.log({ result });
        setListBookingReferral(result?.data?.data)
        ...
        <ScrollView style={{}} contentContainerStyle={{ flexGrow: 1 }}>
            ...
            </View> */}
            {
                isLoading ?
                    ...
                    <>
                        {
                            listBookingReferral?.length > 0 ?
                                <>
                                    {
                                        listBookingReferral?.map((item, index) => {
                                            return (
                                                <View style={{ paddingLeft: _moderateScale(8 * 2), paddingRight: _moderateScale(4) }} key={item?._id}>
                                                    <View style={{ flexDirection: 'row', paddingVertical: _moderateScale(8 * 1.5) }} >
                                                        <View style={{ flex: 4 ,paddingRight:_moderateScale(8*2)}}>
                                                            <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                                                                <Text
                                                                    numberOfLines={1}
                                                                    style={{ ...stylesFont.fontNolanBold, fontSize: _moderateScale(15), flex: 1 }}>
                                                                    {index + 1}. {item?.partnerName}
                                                                </Text>
                                                                <View style={styles.tagLevel}>
                                                                    <Text style={styles.tagLevel__text}>
                                                                        Level {item?.referralAncestorArr?.find(itemFind=> itemFind?.code == infoUserRedux?.collaboratorCode)?.level}
                                                                    </Text>
                                                                </View>
                                                            </View>
                                                            ...
                                                            <Text style={{ ...stylesFont.fontNolan500, fontSize: _moderateScale(14), color: Color.BLACK, marginLeft: _moderateScale(8 * 1), marginTop: _moderateScale(4) }}>
                                                                {item?.servicesNeedCare?.map((itemMap, indexMap) => {
                                                                    if (indexMap == item?.servicesNeedCare?.length - 1) {
                                                                        return `${itemMap?.name}`
                                                                    }
                                                                    return `${itemMap?.name}, `
