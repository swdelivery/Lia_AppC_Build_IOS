...
import { StyleSheet, Platform, View, Text, TouchableOpacity, Image, TextInput, ScrollView, Clipboard, ImageBackground } from 'react-native';
import { _moderateScale } from '../../../Constant/Scale';
import { WHITE, BASE_COLOR, FIFTH_COLOR, BTN_PRICE, GREEN_SUCCESS, GREY, BG_SERVICE, BLACK_OPACITY_8, BLUE_FB, BG_GREY_OPACITY_5, RED, BG_GREY_OPACITY_2 } from '../../../Constant/Color';
...
const ModalInfoBooking = memo((props) => {
    ...
    const [configDataBank, setConfigDataBank] = useState({})
    ...
    const _getconfigfile = async() => {
        let result = await getConfigData("INFO_BANK");
        if(result?.isAxiosError) return

        // console.log(JSON.parse(result?.value?.value));

        setConfigDataBank(result)
    }
    ...
    const _handleConfirmCreatePaymentRequest = async () => {
        ...
        if (currTab == '1') {
            ...
            props?.confirmCreate(dataFetchCreatePaymentRequest)
                ...
                >
                    ...
                    <View style={[styleElement.rowAliCenter, { marginTop: _moderateScale(8 * 3) }]}>
                        ...
                        {/* <TouchableOpacity
                            ...
                            onPress={() => {
                                alertCustomNotAction(`Thông báo`,`Tính năng sẽ sớm ra mắt.`)
                            }}
                    ...
                    {
                        currTab == '1' ?
                            <View style={{ height: _moderateScale(8 * 38) }}>
                                ...
                                <Text style={[stylesFont.fontNolanBold, { fontSize: _moderateScale(14), marginTop: _moderateScale(8 * 2), color: BG_SERVICE }]}>
                                    {configDataBank?.value?.name}
                                </Text>
                                <Text style={[stylesFont.fontNolan, { fontSize: _moderateScale(14), marginTop: _moderateScale(0), color: BLACK_OPACITY_8 }]}>
                                {configDataBank?.value?.branch}
                                </Text>
                                <View style={[styleElement.rowAliCenter, { marginTop: _moderateScale(4) }]}>
                                    <View style={[styleElement.rowAliCenter,{ marginTop: _moderateScale(4), flex:1}]}>
                                        <Text style={[stylesFont.fontNolanBold, { fontSize: _moderateScale(14), color: BG_SERVICE }]}>
                                            {`STK: `}
                                        </Text>
                                        <Text selectable style={[stylesFont.fontNolanBold, { fontSize: _moderateScale(14), color: BG_SERVICE }]}>
                                            {`${configDataBank?.value?.stk}`}
                                        </Text>
                                    </View>
                                    <TouchableOpacity
                                    onPress={() => {
                                        Clipboard.setString(`${configDataBank?.value?.stk}`)
                                    }}
                                ...
                                <TextInput
                                   ...
                                   * Khách hàng cần đặt cọc tối thiểu 10% {`\n`} giá trị dịch vụ để nhận ưu đãi.
                                    </Text>
                                                ...
                                                return (
                                                    ...
                                                    <View style={[{
                                                        ...
                                                        <ImageBackground
                                                            ...
                                                            imageStyle={{borderWidth:1,borderRadius: _moderateScale(8),backgroundColor:BG_GREY_OPACITY_2,}}
                                                            source={{
                                                                uri: item?.path
                                                            }}
                                                        />
