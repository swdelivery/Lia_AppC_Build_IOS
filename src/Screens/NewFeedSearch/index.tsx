...
import {
  getAllPostSearch,
  getMoreAllPostSearch,
  getPostSearchSuggestion,
...
const NewsFeed = (props) => {
  ...
  const listPostRedux = useSelector(
    (state) => state.postReducer?.listPostSearch
  );
  ...
  const _awesomeChildListKeyExtractor = useCallback(
    (item) => `awesome-child-key-${item.id}-${randomStringFixLengthCode(4)}`,
    []
  );
  ...
  const _handleEndReached = () => {
    ...
    dispatch(
      getMoreAllPostSearch(data, null, setFlagLoadMorePost, _handlePage)
    );
  };
  ...
  return (
    <Screen backgroundColor={WHITE} safeTop>
      <HeaderSearch
        keySearch={searchKey}
