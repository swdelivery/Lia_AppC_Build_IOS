...
const CardLevel = ({ data, setShowModalInfoRanked, showPolicy }: Props) => {
  ...
  const isHighestLevel = useMemo(() => {
    let findIndexCurrPartnerLevel = listPartnerLevel?.findIndex(item => item?.code == data.code);
    if (findIndexCurrPartnerLevel !== -1 && findIndexCurrPartnerLevel == listPartnerLevel?.length - 1) {
      return true
    } else {
      return false
    }
