...
import {
  updateDailyDiary,
  updateDailyDiaryv2,
  getTreatmentDiaryIncompleteDaily,
  getPartnerDiaryByEntityId,
  getPartnerDiaryv2,
} from "../../../Redux/Action/Diary";
...
const itemDate = memo(function itemDate(props) {
    ...
    if (props?.serviceDiaryGuide?.guideCodeArr?.length > 0) {
      ...
      setPlayingYoutube((old) => {
        return {
          ...old,
          show: false,
          playList: props?.serviceDiaryGuide?.guideArr?.map(
            (item) => item?.content
          ),
          playListStartIndex: 0,
        };
  ...
  const _handleDelImage = (type, action, path) => {
    if (type === "other" && action === "del") {
      setListImage((old) => [
        ...old.filter((item) => item.image.path !== path),
      ]);
    }
    let tempDataMain = cloneDeep(dataMain);
    tempDataMain.images = [
      ...listImage
        ?.filter((item) => item.image.path !== path)
        ?.map((item) => {
          return {
            type: item?.type,
            image: item?.image?._id,
  ...
  const _handleCompleteAlert = async (isComplete) => {
    ...
    let findPartnerDiary = resultGetPartnerDiaryv2?.data?.data?.find(
      (item) => item?.entityId == props?.data?.treatmentDetailId
    );
    if (findPartnerDiary?._id) {
      navigation.goBack();
      navigation.navigate(ScreenKey.EDIT_DIARY, {
        diaryId: findPartnerDiary?._id,
      });
    }
    ...
    <>
      ...
      <ImageView
        images={listImagesSeeCurr?.map((item) => {
          ...
          return {
            uri: `${
              _get(item, "image.id", "") !== "" ? `${URL_ORIGINAL}/` : ""
            }${item?.image?.path}`,
          };
        })}
        onRequestClose={() => {
      ...
      <KeyboardAwareScrollView showsVerticalScrollIndicator={false}>
        ...
        <ScrollView
          ...
          horizontal
        >
          {props?.serviceDiaryGuide?.guideArr
            ?.filter((itemFilter) => itemFilter?.contentType == "youtubeLink")
            ?.map((item, index) => {
              return (
                <TouchableOpacity
                  onPress={() => {
                ...
                >
                  ...
                  >
                    ...
                    >
                      ...
                      <Image
                        style={sizeIcon.xxlllg}
                        source={require("../../../NewIcon/circlePlay.png")}
                      />
                    </ImageBackground>
                  </View>
                  <View
        ...
        {props?.serviceDiaryGuide?.guideArr
          ?.filter((itemFilter) => itemFilter?.contentType == "html")
          ?.map((item, index) => {
            return (
        ...
        <View style={{ paddingHorizontal: _moderateScale(8 * 2) }}>
          <View style={{}}>
            <Text style={[styles.titSection, { color: BLUE_FB }]}>
              Hình ảnh
            </Text>
          </View>
          <Text
          ...
          >
            *Để đảm bảo quyền lợi và hiệu quả thẩm mỹ cuối cùng, khách hàng cần
            cập nhật đầy đủ nhật ký hẫu phẫu từng ngày theo đúng hướng dẫn.
          </Text>
        </View>
        ...
        >
          <View style={[styles.itemImage]}>
              ...
              return item.type === "left" ? (
                ...
                >
                  ...
                  <TouchableOpacity
                    ...
                    onPress={() =>
                      _handleDelImage("other", "del", item?.image?.path)
                    }
                    style={{
                      position: "absolute",
            ...
            {_findIndex(listImage, { type: "left" }) < 0 ? (
              <TouchableOpacity
                onPress={() => _handlePickImage("left")}
          ...
          <View style={[styles.itemImage]}>
            ...
            {_findIndex(listImage, { type: "front" }) < 0 ? (
              <TouchableOpacity
                onPress={() => _handlePickImage("front")}
                style={[styles.btnAddImage, { marginTop: _moderateScale(8) }]}
              >
          ...
          <View style={[styles.itemImage]}>
            ...
            {_findIndex(listImage, { type: "right" }) < 0 ? (
              <TouchableOpacity
                onPress={() => _handlePickImage("right")}
                style={[styles.btnAddImage, { marginTop: _moderateScale(8) }]}
              >
          ...
          <Text style={[styles.textImage]}>
            Khác{" "}
            {
              <Text
                style={[
                  styles.textImage,
                  { ...stylesFont.fontNolan, color: GREY },
                ]}
              >
                (Không bắt buộc)
              </Text>
        ...
        >
          ...
          <TouchableOpacity
            ...
            style={[
              styles.btnAddImage,
              { marginTop: _moderateScale(0), marginHorizontal: 0 },
            ]}
          >
            <Image
              style={[sizeIcon.lg]}
    ...
    transform: [
      {
        translateY: scrollA.interpolate({
          inputRange: [
            ...
            0,
            _moderateScale(130),
            _moderateScale(130) + 1,
          ],
          outputRange: [
            -_moderateScale(130) / 2,
            0,
            _moderateScale(130) * 0.75,
            _moderateScale(130) * 0.75,
          ],
        }),
      },
