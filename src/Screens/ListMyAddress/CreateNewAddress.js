...
const CreateNewAddress = memo((props) => {
    ...
    const _confirmCreatePickupAddress = async () => {
        ...
        if (result?.isAxiosError) return
        navigation.navigate(CREATE_BOOKING, { currPickUpAddress: result?.data?.data })
    }
    ...
    return (
        <View style={styles.container}>
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', paddingHorizontal: _moderateScale(8 * 2), marginTop: _moderateScale(8 * 2) }}>
                <View>
                    <Text style={{ ...stylesFont.fontNolan500, color: GREY, fontSize: _moderateScale(14) }}>
                        Khu vực được chọn
                    </Text>
                </View>
                ...
                <View>
                    <TouchableOpacity onPress={() => {
                        ...
                        <Text style={{ ...stylesFont.fontNolan500, color: ORANGE, fontSize: _moderateScale(14) }}>
                            Thiết lập lại
                        </Text>
                    </TouchableOpacity>
                </View>
                ...
                currChoiceProvince?.name ?
                    <View style={{ flexDirection: 'row', marginTop: _moderateScale(8 * 2) }}>
                        <View style={{ width: _moderateScale(2), backgroundColor: BG_GREY_OPACITY_5, alignItems: 'center', marginLeft: _moderateScale(8 * 2) }} >
                            <View style={{ width: _moderateScale(8), height: _moderateScale(8), backgroundColor: BLUE_FB, borderRadius: _moderateScale(4), position: 'absolute', top: _moderateScale(6) }} />
            ...
            }
            {
                currChoiceDistrict?.name ?
                    <View style={{ flexDirection: 'row' }}>
                        ...
                        <View style={{ flex: 1, marginLeft: 10, paddingHorizontal: _moderateScale(8 * 1) }}>
                            <View style={{ minHeight: _moderateScale(8 * 4) }}>
                                <Text style={{ fontSize: _moderateScale(14), color: BLUE_FB }}>{currChoiceDistrict?.name}</Text>
                            </View>
                        </View>
                    </View>
                    : <></>
                    ...
                    <View style={{ flexDirection: 'row' }}>
                        ...
                        <View style={{ flex: 1, marginLeft: 10, paddingHorizontal: _moderateScale(8 * 1) }}>
                            <View style={{ minHeight: _moderateScale(8 * 2) }}>
                                <Text style={{ fontSize: _moderateScale(14), color: BLUE_FB }}>{currChoiceWard?.name}</Text>
                            </View>
                        </View>
                    </View>
                    : <></>
            ...
            <Collapsible collapsed={currChoiceWard?.name ? false : true}>
                <View style={[styleElement.rowAliCenter, { paddingHorizontal: _moderateScale(8 * 0), marginTop: _moderateScale(8 * 2) }]}>
                    <TextInput
                        ref={RefTextInput}
                        onChangeText={(e) => setTextAddress(e)}
                        value={textAddress}
                        style={[styles.inputCodeRef, { backgroundColor: BG_GREY_OPACITY_2, flex: 1, marginHorizontal: _moderateScale(8 * 2) }]} placeholder={"vd: Hẽm 401 Cách mạng tháng 8"} />
                </View>
            ...
            {/* <Collapsible collapsed={currChoiceWard?.name ? true : false}> */}
            {
                !currChoiceWard?.name ?
                    <>
                        <View style={{ marginTop: _moderateScale(8 * 2), height: _moderateScale(8 * 4), backgroundColor: BG_GREY_OPACITY_5, paddingHorizontal: _moderateScale(8 * 2), justifyContent: 'center' }}>
                            <Text style={{ ...stylesFont.fontNolanBold, color: GREY, fontSize: _moderateScale(14) }}>
                                {
                                    listWard?.length > 0 ?
                                        `Phường/Xã`
                                        :
                        ...
                        <ScrollView ref={RefScrollView} scrollIndicatorInsets={{ right: 1 }} contentContainerStyle={{ alignItems: 'center' }}>
                            {
                                listWard?.length > 0 ?
                                    <>
                                        {
