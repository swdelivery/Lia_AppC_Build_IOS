...
const FeedBackDoctor = memo((props) => {
    ...
    useEffect(() => {
        if (props?.route?.params?.currentDoctor?._id) {
            _getReview()
        }
    }, [props?.route?.params?.currentDoctor])

    const _getReview = async () => {
        var review = await getDoctorReviewByCode(props?.route?.params?.currentDoctor?.code)
    ...
    const _getReviewMore = async (_idMore) => {
        setIsLoadMore(true)
        var review = await getDoctorReviewByCode(props?.route?.params?.currentDoctor?.code, _idMore)
