...
import { BASE_COLOR, BG_BEAUTY, BG_GREY_OPACITY_3, BG_GREY_OPACITY_5, BG_GREY_OPACITY_7, BLACK_OPACITY_8, BLUE, BLUE_FB, BLUE_OCEAN, BLUE_TITLE, GREY, GREY_FOR_TITLE, MAIN_OPACITY_8, RED, SECOND_COLOR, THIRD_COLOR, WHITE } from '../../Constant/Color';
...
const DiaryOfNewsFeed = (props) => {
    ...
    useEffect(() => {
        ...
        console.log('props', props?.route?.params?.idPost)
        // dispatch(getPostById(currentPostRedux?._id))
        if (props?.route?.params?.idPost) {
            dispatch(getPostById(props?.route?.params?.idPost))
        }
    ...
    return (
        <KeyboardAvoidingView keyboardVerticalOffset={Platform.OS == 'ios' &&
            ...
            <View style={styles.container}>
                <StatusBarCustom />
                <View style={{ flex: 1, backgroundColor: BG_BEAUTY }}>
                    <View style={[styleElement.rowAliCenter, {
                        backgroundColor: BASE_COLOR,
                        justifyContent: 'space-between', paddingHorizontal: _moderateScale(8 * 3), paddingVertical: _moderateScale(8 * 1.86)
                    }]}>
                        <TouchableOpacity
