...
import { getPartnerDiaryById } from '../../../Redux/Action/PartnerDiary';
...
const ItemFeed = memo(function ItemFeed(props) {
    ...
    useEffect(() => {
        if(!isEmpty(currentPostRedux))
        {
            setContent(currentPostRedux?.content)
            if(currentPostRedux?.images.length>0)
            {
                setListImage(currentPostRedux?.images)
                setShareImage(true)
            }

            if(currentPostRedux?.template?.type === "PartnerDiary_TreatmentDetail"&&
                !isEmpty(currentPostRedux?.template?.data?.partnerDiaryId)
            )
            {   

                dispatch(getPartnerDiaryById(currentPostRedux?.template?.data?.partnerDiaryId))
                var temp = {...currentTreatmentDiaryRedux}
                dispatch({
            ...
            setHiddenIdArr(currentPostRedux?.hiddenIdArr)
        }
    }, [currentPostRedux])
   ...
   const _handlePost = async () =>{
    ...
    var data = {
            "content": content,
            "images": [...listIdImageHasUpload,...imageServer],
            "template": {
        ...
        console.log({data}) 
        dispatch(updatePost(currentPostRedux?._id,data))
    }
