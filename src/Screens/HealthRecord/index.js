...
import { TouchableOpacity, TextInput, Animated, Image, StyleSheet, Text, View, FlatList } from 'react-native';
import ImageView from "react-native-image-viewing";
import { navigation } from '../../../rootNavigation';
import SocketInstance from '../../../SocketInstance';
import FastImage from '../../Components/Image/FastImage';
import { BASE_COLOR, BG_GREY_OPACITY_2, GREY, GREY_FOR_TITLE, SECOND_COLOR, WHITE, BG_GREY_OPACITY_5, BLACK_OPACITY_8, BTN_PRICE, RED } from '../../Constant/Color';
...
import { getHealthRecord, updateHealthRecord } from '../../Redux/Action/ProfileAction';
import { useSelector, useDispatch } from "react-redux";
import { get as _get, findIndex as _findIndex, get, findIndex } from 'lodash'
...
const Profile = props => {
    ...
    const [bloodGroup, setBloodGroup] = useState(["O+", "O-", "A+", "A-", "B+", "B-", "AB+", "AB-"])
    const [oldArr, setoldArr] = useState(Array.from(new Array(100), (x, i) => i + 1))
    const [indexOld, setIndexOld] = useState(0)

    const [heightArr, setHeightArr] = useState(Array.from(new Array(120), (x, i) => i + 135))
    const [weightArr, setWeightArr] = useState(Array.from(new Array(220), (x, i) => i + 1))
    ...
    useEffect(() => {
        ...
        if(healthRecordStatus?.basicInfo?.bloodGroup){
            ...
            setTimeout(() => bloodFlatListRef.current.scrollToIndex({ animated: true, index: blood }), 100);
        }
       
        if(healthRecordStatus?.basicInfo?.age){
            var old = oldArr.indexOf(healthRecordStatus?.basicInfo?.age)
            setTimeout(() => oldFlatListRef.current.scrollToIndex({ animated: true, index: old }), 200);
        }

        if(healthRecordStatus?.basicInfo?.height){
            var height = heightArr.indexOf(Math.floor(healthRecordStatus?.basicInfo?.height))
            setTimeout(() => heightFlatListRef.current.scrollToIndex({ animated: true, index: height }), 300);
        }
        
        if(healthRecordStatus?.basicInfo?.weight){
            var weight = weightArr.indexOf(Math.floor(healthRecordStatus?.basicInfo?.weight))
            setTimeout(() => weightFlatListRef.current.scrollToIndex({ animated: true, index: weight }), 400);
        }
    ...
    const _awesomeChildListKeyExtractorBlood = useCallback((item) => `awesome-child-blood-${item}`, []);
    ...
    const _awesomeChildListKeyExtractorOld = useCallback((item) => `awesome-child-old-${item}`, []);
    ...
    const _awesomeChildListKeyExtractoHeight = useCallback((item) => `awesome-child-height-${item}`, []);
    ...
    const _awesomeChildListKeyExtractorWeight = useCallback((item) => `awesome-child-weight-${item}`, []);

    const getItemLayout = (data, index) => {
        return { length: _moderateScale(46), offset: _moderateScale(46) * index, index };
    }
    ...
    return (
        <View style={styles.container}>
            ...
            <Animated.ScrollView
                ...
                <ImageView
                    images={[`${URL_ORIGINAL}${infoUserRedux?.infoUser?.profile?.fileAvatar?.link}`].map(item => {
                        return {
                            uri: item,
                        }
                    })}
                ...
                <View style={{
                    ...
                    <View style={styles.absoluteTitle}>
                        <Image style={[sizeIcon.lg, { marginRight: _moderateScale(8 * 2) }]} source={require('../../Icon/protect.png')} />
                        <Text style={[stylesFont.fontNolanBold,
                        { color: BASE_COLOR }]}>DỮ LIỆU SỨC KHOẺ</Text>
                    </View>
                    <KeyboardAwareScrollView >
                        ...
                        <View style={[styleElement.rowAliCenter, { marginTop: _moderateScale(8 * 3), paddingHorizontal: _moderateScale(32), justifyContent: 'space-between' }]}>
                            <View>
                                <Text style={[stylesFont.fontNolan500, styles.titleMain, { marginLeft: 0, marginBottom: _moderateScale(16) }]}>
                                    Huyết áp
                                </Text>
                                <TextInput
                        ...
                        <View style={{ marginTop: _moderateScale(8 * 3) }}>
                            <Text style={[stylesFont.fontNolan500, styles.titleMain, { marginLeft: _moderateScale(32) }]}>
                                Tiền sử bệnh
                            </Text>
                        </View>
                                ...
                                healthRecord?.map((item, index) => {
                                    ...
                                    if (_get(check, item.code, false) === true) {
                                        return (
                                            <TouchableOpacity
                                                ...
                                                style={[styleElement.rowAliCenter, { marginBottom: _moderateScale(8) , paddingVertical:_moderateScale(2)}]}>
                                                <Image style={[sizeIcon.lg]} source={require('../../NewIcon/acheck.png')} />
                                                <Text style={[stylesFont.fontNolan500, { marginLeft: _moderateScale(8), fontSize: _moderateScale(14), flex: 1, color: BLACK_OPACITY_8 }]}>
                                                    {item?.value}
                                                </Text>
                                            </TouchableOpacity>
                                        )
                                    ...
                                    return (
                                        <TouchableOpacity
                                            ...
                                            style={[styleElement.rowAliCenter, { marginBottom: _moderateScale(8) ,paddingVertical:_moderateScale(4)}]}>
                                            <Image style={[sizeIcon.lg]} source={require('../../NewIcon/icheck.png')} />
                                            <Text style={[stylesFont.fontNolan500, { marginLeft: _moderateScale(8), fontSize: _moderateScale(14), flex: 1, color: BLACK_OPACITY_8 }]}>
                                                {item?.value}
                                            </Text>
                                        </TouchableOpacity>
