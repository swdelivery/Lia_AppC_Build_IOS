...
import {
    ...
    TouchableOpacity, View, FlatList, Platform, KeyboardAvoidingView
} from 'react-native';
import { useDispatch, useSelector } from 'react-redux';
import { navigation } from '../../../rootNavigation';
import { BASE_COLOR, BG_BEAUTY, BG_GREY_OPACITY_3, BG_GREY_OPACITY_5, BG_GREY_OPACITY_7, BLACK_OPACITY_8, BLUE, BLUE_FB, BLUE_OCEAN, BLUE_TITLE, GREY, GREY_FOR_TITLE, MAIN_OPACITY_8, SECOND_COLOR, THIRD_COLOR, WHITE, BG_GREY_OPACITY, RED } from '../../Constant/Color';
...
import { getAllPost, getMoreAllPost, getCommentsByPostId, createPostCommentv2 } from '../../Redux/Action/PostAction';
...
const ModalCommentPost = memo((props) => {
    ...
    useEffect(() => {
        ...
        if (isFirstLoaded && listComments?.length > 0) {
            ...
            let indexFind = listComments?.findIndex(item => item?._id == props?.route?.params?.idCommentHighlight);
            if (indexFind !== -1) {
                setTimeout(() => {
                    listCommentRef.current.scrollToIndex({ animated: true, index: indexFind })
    ...
    const _comfirmSendReplyComment = async (refTextInput) => {
        ...
        let indexCommentParent = tempListComments?.findIndex(item => item?._id == result?.data?.data?.parentId);
        ...
        <>
            ...
            <KeyboardAvoidingView keyboardVerticalOffset={Platform.OS == 'ios' && getBottomSpace() == 0 ? _heightScale(0) : 0} behavior={Platform.OS == 'ios' ? 'padding' : null} style={{
                ...
                listComments?.map((item, index) => {
                                ...
                                return (
                                    <View style={{ marginLeft: _moderateScale(8 * 3) }}>
                                        ...
                                        <TouchableOpacity onPress={_getMoreComments}>
                                            <Text style={{ ...stylesFont.fontNolan500, fontSize: _moderateScale(14) }}>Tải thêm bình luận</Text>
                                        </TouchableOpacity>
                    ...
                    <View style={{ borderTopWidth: _moderateScale(1), borderColor: BG_GREY_OPACITY_5 }}>

                        {
                            !isEmpty(currReplyComment) ?
                                <>
                                    <View style={[styleElement.rowAliCenter, { marginTop: _moderateScale(8), marginLeft: _moderateScale(8 * 4) }]}>
                                        <View style={{ width: _moderateScale(2), height: _moderateScale(16), backgroundColor: RED, marginRight: _moderateScale(4) }} />
                        ...
                        <View style={{
                            width: _width, paddingVertical: _moderateScale(12),
