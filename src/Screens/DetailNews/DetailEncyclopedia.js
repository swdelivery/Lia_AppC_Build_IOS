...
import { getNewsById, getEncyclopediaByID } from '../../Redux/Action/News';
...
const DetailEncyclopedia = (props) => {
    ...
    const _getService = async () => {
        var currentService = await getEncyclopediaByID(props?.route?.params?.idNews)
        if (currentService?.isAxiosError) return
        setCurrService(currentService?.data?.data)
...
export default DetailEncyclopedia;