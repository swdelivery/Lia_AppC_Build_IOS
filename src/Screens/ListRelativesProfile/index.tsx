...
import { deletePartnerRelative, getListPartnerRelative } from '@Redux/relatives/actions'
...
const ListRelativesProfile = () => {
  ...
  const getData = () => {
    dispatch(getListPartnerRelative.request({
      condition: {
        "partnerId": {
          "equal": infoUser?._id
        }
  ...
  const _handleDelete = (data) => {
    showConfirmation(
      ...
      () => {
        dispatch(deletePartnerRelative.request({
          _id: data?._id
        }))
      }
    );
    ...
    return (
      <TouchableOpacity
        ...
        <Row
          gap={8 * 2}
          padding={8 * 2}>
          <Avatar size={8 * 8} avatar={item?.avatar?._id ? item?.avatar : null} circle />
          <Column gap={8} flex={1}>
