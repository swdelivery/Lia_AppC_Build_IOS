...
import {
  getCurrentPartnerTrackingWeight,
  getOnePartnerTrackingWeight,
  getDataPartnerFood,
  getDataPartnerActivity,
  updatePartnerTrackingWeight,
  getCurrPartnerWeightGoal,
  getAssetsLoseWeight,
...
const index = memo((props) => {
  ...
  const dateFilterTrackingWeightRedux = useSelector(
    (state) => state.loseWeightReducer?.dateFilterTrackingWeight
  );
  const foodPartnerMenuRedux = useSelector(
    (state) => state.loseWeightReducer?.foodPartnerMenu
  );
  const activityPartnerMenuRedux = useSelector(
    (state) => state.loseWeightReducer?.activityPartnerMenu
  );
  const partnerTrackingWeightRedux = useSelector(
    (state) => state.loseWeightReducer?.partnerTrackingWeight
  );
  ...
  const _handleUpdateEmotion = async (item) => {
    ...
    if (!listEmotionTemp.includes(item?.code)) {
  ...
  return (
    <View style={styles.container}>
      ...
      <KeyboardAwareScrollView>
        ...
        >
          <Text
            style={[
              ...
              { fontSize: _moderateScale(16), color: BLACK_OPACITY_8, flex: 1 },
            ]}
          >
            Thực đơn hằng ngày
          </Text>
          <Text
            style={[
              ...
              { fontSize: _moderateScale(12), color: BLACK_OPACITY_8 },
            ]}
          >
            (Đơn vị tính: Carlory)
          </Text>
        ...
        >
          <ItemMenu
            data={foodPartnerMenuRedux}
            clickAdd={_handleAddFoodMorning}
            flag={"morning"}
            title={"Buổi sáng"}
          />
...
?.filter(item => item?.session == 'snack') */}
        ...
        >
          ...
          {moment(new Date()).isSame(
            moment(dateFilterTrackingWeightRedux),
            "day"
          ) ? (
            <TouchableOpacity
              onPress={() => {
        ...
        >
          ...
          >
            {listAssetLoseWeight?.map((item, index) => {
              if (
                partnerTrackingWeightRedux?.emotionCodeArr?.find(
                ...
                return (
                  ...
                  >
                    <Text
                      style={[
                        stylesFont.fontNolan,
                        { fontSize: _moderateScale(14), color: BLACK },
                      ]}
                    >
                      {item?.name}
                    </Text>
