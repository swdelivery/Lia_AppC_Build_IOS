...
import {
  getDetailCampainState,
  getListCompanionRequestState,
} from "@Redux/charity/selectors";
...
const Companion = () => {
  ...
  const {
    data: { createBy, name, fundTarget, _id, avatar },
  } = useSelector(getDetailCampainState);
  const { data: listCompanionRequest } = useSelector(
    getListCompanionRequestState
  );
  ...
  const _handleSetMoneyByPercent = useCallback(
    (percent) => () => {
  ...
  const _handleConfirm = useCallback(() => {
    ...
    if (!checkIsMoney) {
      ...
      showConfirmation(
        ...
        async () => {
          dispatch(
            createVolunteerCompanion.request({
              volunteerId: _id,
  ...
  return (
    <Screen safeBottom backgroundColor={BACKGROUND_COLOR}>
      ...
      <ScrollView>
        <Column gap={8 * 2} alignItems="center">
          <Column marginTop={8 * 2} alignSelf="center">
            <Text weight="bold">Đồng hành cùng</Text>
          </Column>

          <Avatar circle size={8 * 8} avatar={avatar} />

          <Text color={NEW_BASE_COLOR} weight="bold">
            {createBy.toUpperCase()}
          </Text>

          <Column gap={4} marginHorizontal={8 * 2} alignItems="center">
            <Text fontStyle="italic">Trong chiến dịch</Text>
        ...
        {hasRequested ? (
          <Column gap={8} margin={8 * 4}>
            <Text fontStyle="italic" style={{ textAlign: "center" }}>
              Chân thành cảm ơn sự đồng hành của bạn cho chiến dịch gây quỹ. Đơn
              đăng ký của bạn đã được nhận và
              <Text weight="bold">
                {` đang được chúng tôi tiến hành xử lý.`}
              </Text>
            </Text>
            <Text fontStyle="italic" style={{ textAlign: "center" }}>
              Xin vui lòng kiên nhẫn chờ đợi, chúng tôi sẽ nhanh chóng xem xét
              và thông báo kết quả đến bạn!
            </Text>
            <Text fontStyle="italic" style={{ textAlign: "center" }}>
          ...
          <Column gap={8} marginTop={8 * 2} paddingHorizontal={8 * 2}>
            <Text weight="bold">Mục tiêu kêu gọi đồng hành</Text>
            <Text color={GREY}>Nhập số tiền kêu gọi đồng hành</Text>
            ...
            <Column justifyContent="center">
              <TextInput
                ...
                placeholder="0"
              />
              <Text color={GREY} weight="bold" style={styles.absoluteText}>
                VND
              </Text>
            </Column>
