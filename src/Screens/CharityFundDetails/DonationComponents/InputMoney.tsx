...
const InputMoney = () => {
  ...
  const _handleSetMoney = useCallback(
    (value) => () => {
      dispatch(
        selectAmount(value.toString().replace(/\B(?=(\d{3})+(?!\d))/g, "."))
      );
  ...
  return (
    <Column gap={8 * 2} marginTop={8 * 2}>
      ...
      <Column marginHorizontal={8 * 2} justifyContent="center">
        <TextInput
      ...
      <Row marginTop={8 * 1} paddingHorizontal={8 * 2} gap={8}>
        <TouchableOpacity
          onPress={_handleSetMoney(50000)}
          style={styles.btnPercent}
        >
          <Text color={"#888888"} weight="bold">
            50.000
          </Text>
        </TouchableOpacity>
        <TouchableOpacity
