...
const Banner = () => {
  ...
  useEffect(() => {
    ...
    if (listNews?.length > 0) {
      const file = head(listNews[currIndexBanner]?.representationFileArr);
      if (file) {
        getColors(getImageAvataUrl(file)).then(() => {
  ...
  const _renderImage = ({ item, index }: RenderItemProps<News>) => {
    return (
      <EachImage
        handlePress={() => {
  ...
  const { keyExtractor } = useItemExtractor<News>((item) => item._id);
