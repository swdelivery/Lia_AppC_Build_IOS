...
import { BG_GREY_OPACITY_5, BG_GREY_OPACITY_7, BLACK, BLACK_OPACITY_4, BORDER_COLOR, GREY, NEW_BASE_COLOR, RED, WHITE } from '@Constant/Color'
import { _moderateScale, _width } from '@Constant/Scale'
import { styleElement } from '@Constant/StyleElement'
import { URL_ORIGINAL } from '@Constant/Url'
import ScreenKey from "@Navigation/ScreenKey"
import { createReviewTreatment, getReviewByTreatmentDetailForPartner, uploadModule } from '@Redux/Action/BookingAction'
import { isEmpty } from 'lodash'
import React, { useCallback, useEffect, useMemo, useState } from 'react'
import { Image, KeyboardAvoidingView, ScrollView, StyleSheet, TextInput, TouchableOpacity, View } from 'react-native'
...
const ServiceReview = () => {
  ...
  const { data: { treatmentDetailId, review, serviceName } } = useNavigationParams<ScreenKey>();
  ...
  const [reaction, setReaction] = useState({ code: "VERY_GOOD", name: 'Tuyệt vời' })
  ...
  const _getReview = useCallback(async (_id) => {
    ...
    setListComplains([
      { code: "serviceReview", name: "Chất lượng dịch vụ", isComplained: serviceReview?.isComplained },
      { code: "securityReview", name: "Bảo vệ", isComplained: securityReview?.isComplained },
      { code: "receptionReview", name: "Lễ tân", isComplained: receptionReview?.isComplained },
      { code: "consultantReview", name: "Tư vấn viên", isComplained: consultantReview?.isComplained },
      { code: "staffReview", name: "Bác sĩ", isComplained: staffReview?.isComplained },
      { code: "csReview", name: "Chăm sóc khách hàng", isComplained: csReview?.isComplained },
      { code: "branchReview", name: "Chi nhánh", isComplained: branchReview?.isComplained },
    ])
    setDescription(serviceReview?.comment)
    setImagesReview(serviceReview?.imageReview?.map(item => {
      return {
        ...item,
  ...
  const _handleChoiceReaction = useCallback((reaction) => () => {
  ...
  const _handleRemoveImage = useCallback((image) => () => {
  ...
  const isComplained = useCallback((code) => {
    return listComplains?.find(item => item?.code == code)?.isComplained ? true : false
  }, [listComplains])
    ...
    return (
      <TouchableOpacity
        ...
        <Column
          ...
          <Text
            color={data?.isComplained ? WHITE : BLACK}>
            {data?.name}
          </Text>
        </Column>
      </TouchableOpacity>
      ...
      >
        ...
        <ScrollView>
                  ...
                  if (index <= indexCurrReaction) {
                    ...
                    return (
                      <TouchableOpacity
                        disabled={hasReview}
                        onPress={_handleChoiceReaction(item)}>
                        <Icon size={8 * 5} name='star' color={BG_GREY_OPACITY_7} />
                      </TouchableOpacity>
                    )
                  }
                })
          ...
          <Column alignItems='center'>
            <Text>
              Điều gì khiến bạn <Text weight='bold' color={RED}>KHÔNG</Text> hài lòng?
            </Text>
          </Column>
          <Row
            marginTop={8 * 3}
            ...
            flexWrap='wrap'>
            {
              listComplains.map((item, index) => {
                return <ItemComplain disable={hasReview} key={index} data={item} />
              })
            }
          </Row>
