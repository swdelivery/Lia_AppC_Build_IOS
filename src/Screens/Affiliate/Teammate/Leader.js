...
import { getWallet, getCurrentCommission, getCollabStatistic, getTeammateParent, getTeammateSuperior, getTeammateLeader } from '../../../Redux/Action/InfoAction'
...
const Leader = memo((props) => {

    const [listSuperior, setListSuperior] = useState([1, 2, 3, 4, 5, 5, 6, 7, 7, 5, 4, 4, 4])

    const [teammateLeader, setTeammateLeader] = useState({})
    ...
    const _getTeammateLeader = async () => {
        let result = await getTeammateLeader();
        if (result?.isAxiosError) return setIsFirstLoaded(true)
        setTeammateLeader(result?.data?.data);
        setIsFirstLoaded(true)
    }
            ...
            <ScrollView contentContainerStyle={{ flexGrow: 1 }} scrollIndicatorInsets={{ right: 1 }}>
                <View style={{
                    ...
                    <Text style={{
                        ...
                        color: BLACK,
                        fontStyle: 'italic'
                    }}>* Đây là cấp cao nhất trong hệ thống. {`\n`}</Text>
                </View>
                                    ...
                                    <>
                                        <TouchableOpacity
                                            ...
                                            <Text style={{
                                                ...
                                                {teammateLeader?.partner?.phone[0].fullPhoneNumber?.slice(0, 3)}xxxxx{teammateLeader?.partner?.phone[0].fullPhoneNumber?.slice(teammateLeader?.partner?.phone[0].fullPhoneNumber?.length - 4, teammateLeader?.partner?.phone[0].fullPhoneNumber?.length)}
                                            </Text>
                                        </TouchableOpacity>
                                        <Image
                                        ...
                                        {
                                            teammateLeader?._id !== teammateParent?._id ?
                                                    ...
                                                    <View style={{
                                                    ...
                                                    }}>
                                                        {
                                                            teammateSuperior?.length > 0 ?
                                                                ...
                                                                :
                                                                <View style={{
                                                                    width: (_width - _widthScale(8 * 4)) / 1 - 2,
                                                                    justifyContent: 'center',
                                                                    alignItems: 'center',
                                                                    padding: _moderateScale(8 * 1),
                                                                }}>
                                                                    <Text style={{ alignSelf: 'center' }}>
                                                                        * Hiện chưa có cấp trên
                                                                    </Text>
                                                                </View>
                                                        }
                                                    </View>
