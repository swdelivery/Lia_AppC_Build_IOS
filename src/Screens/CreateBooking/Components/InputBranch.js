...
const InputBranch = memo((props) => {
    return (
        <View style={{ paddingHorizontal: _moderateScale(8 * 3), marginTop: _moderateScale(8 * 0) }}>
            ...
            {
                props?.currBranch?._id ?
                    <TouchableOpacity
                        onPress={props?.handleOpenListBranch}
                        style={styles.btnPickBranch}>
                        <View style={{ flexDirection: 'row' }}>
                            <Image style={[sizeIcon.sm]} source={require('../../../NewIcon/building.png')} />
                            {
                                <Text style={{ color: Color.RED, fontSize: _moderateScale(16), marginLeft: _moderateScale(4) }}>*</Text>
                            }
                        </View>
                        <Text style={[styles.btnPickBranch__text, { color: Color.BLACK }]}>
                            {
                                props?.currBranch?.name
                            }
                        </Text>
