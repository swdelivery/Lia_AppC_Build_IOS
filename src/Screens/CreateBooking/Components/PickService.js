  ...
  const _renderTempPrice = (price) => {
    ...
    props?.listCouponActive?.map((item) => {
      if (item?.coupon?.discountType == "percent") {
        ...
        if (
          item?.coupon?.maxAmountDiscount &&
          (price * item?.coupon?.discountAmount) / 100 >
            item?.coupon?.maxAmountDiscount
        ) {
    ...
    <>
      <ModalPickTopping
        confirm={(currChoice, listTopping) => {
          ...
          let indexCurrChoiceInArray = props?.listServiceHasChoice?.findIndex(
            (item) => item?._id == currChoice?._id
          );
      ...
      <View
        style={[
          { marginTop: _moderateScale(0), marginBottom: _moderateScale(8 * 0) },
        ]}
      >
        <ScrollView
                ...
                >
                  ...
                  {props?.flashSale ? (
                    ...
                    <TouchableOpacity
                      ...
                      onPress={() => {
                        props?.setListServiceHasChoice((olds) =>
                          olds?.filter(
              ...
              <TouchableOpacity
                onPress={() => {
                  navigation.navigate(ScreenKey.PICK_SERVICE_TO_BOOKING, {
                    setListServiceHasChoice: props?.setListServiceHasChoice,
                    listServiceHasChoice: props?.listServiceHasChoice,
                  });
                }}
                style={[
                  styles.itemService,
      ...
      {props?.flashSale ? (
        ...
        <Collapsible
          collapsed={props?.listServiceHasChoice?.length > 0 ? false : true}
        >
                  ...
                  <View style={styles.avoucher}>
                    ...
                    >
                      <View style={{ paddingLeft: _moderateScale(8 * 1) }}>
                        <Text
                          numberOfLines={1}
                          style={styles.avoucher__left__text1}
                        >
                          {item?.couponCode}
                        </Text>
                        <Text
                          numberOfLines={1}
                          style={styles.avoucher__left__text2}
                        >
                          {item?.coupon?.name}
                        </Text>
                        <Text
                          numberOfLines={1}
                          style={styles.avoucher__left__text3}
                        >
                          HSD:{" "}
                  ...
                  <View style={styles.voucher}>
                    ...
                    <TouchableOpacity
                      onPress={() => {
                        ...
                        if (
                          item?.coupon?.minRequiredOrderAmount &&
                          props?.listServiceHasChoice?.reduce(
                            (sum, { price }) => sum + price,
                            0
                          ) < item?.coupon?.minRequiredOrderAmount
                        ) {
                          return alertCustomNotAction(
                            `Thông báo`,
                            `Đơn hàng chưa đạt đủ ${item?.coupon?.minRequiredOrderAmount
                              ?.toString()
                              .replace(
                                /\B(?=(\d{3})+(?!\d))/g,
                                "."
                      ...
                      activeOpacity={0.7}
                      style={[styles.voucher__right]}
                    >
                      <View
                        style={[
                          styles.verticalLineDash,
          ...
          >
            Tạm tính:{" "}
            {
              Platform.OS == "ios" ? (
                <Text
                  style={{ color: Color.BLUE_FB, ...stylesFont.fontDinTextPro }}
                >
                  {_renderTempPrice(
                    props?.listServiceHasChoice?.reduce(
                      (sum, { price, listTopping }) => {
                        if (listTopping?.length > 0) {
                          return (
                            sum +
                            price +
                            listTopping?.reduce(
                              (sumTopping, { extraAmount }) => {
