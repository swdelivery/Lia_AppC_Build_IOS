...
const ItemTreatmentDetail = memo((props: any) => {
  ...
  const listDoctorRedux = useSelector(
    (state) => state?.membersReducer?.listDoctor
  );
    ...
    return (
      <View
        style={[
          ...
          { justifyContent: "center", marginLeft: _moderateScale(8) },
        ]}
      >
  ...
  return (
    <>
      ...
      <View style={[styles.main, { gap: 8 * 2 }]}>
        <Column gap={8}>
          <RowInfo name={"Dịch vụ điều trị:"} value={serviceName} />
          {listDoctorRedux?.find((itemFind) => itemFind?.userId == doctorId)
            ?.name ? (
            <RowInfo
              name={"Bác sĩ chính:"}
          ...
          {completeAt ? (
            <RowInfo
              name={"Thời gian:"}
              value={`${moment(created).format("HH:mm")} - ${moment(
                completeAt
              ).format("HH:mm")} | ${moment(created).format("DD/MM/YYYY")}`}
            />
          ) : (
            <RowInfo
              name={"Thời gian:"}
          ...
          <Column gap={8} alignItems="center" flex={1}>
            ...
            >
              <Image
                ...
                source={{
                  uri: `${URL_ORIGINAL}${imageBeforeTreatment[0]?.link}`,
                }}
              />
            </TouchableOpacity>
          </Column>
            ...
            <Column gap={8} alignItems="center" flex={1}>
              ...
              >
                <Image
                  style={styles.imageTreatment}
                  source={{
                    uri: `${URL_ORIGINAL}${imageAfterTreatment[0]?.link}`,
                  }}
                />
              </TouchableOpacity>
            </Column>
        ...
        <Row alignItems="flex-end">
          <Column gap={8} flex={1}>
            ...
            {review?._id ? (
              ...
              <Text>Bạn chưa đánh giá dịch vụ</Text>
            )}
          </Column>
          {review?._id ? (
            <TouchableOpacity
