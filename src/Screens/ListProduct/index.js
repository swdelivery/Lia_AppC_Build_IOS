...
const ListProductSreen = () => {
  ...
  const listProductGroupRedux = useSelector(
    (state) => state.productGroupReducer?.listProductGroup
  );
      ...
      <>
        <TabBar
          ...
          renderLabel={({ route, focused, color }) => (
            ...
            // </View>
            <View
              style={{ width: _moderateScale(8 * 8.9), alignItems: "center" }}
            >
              <View
                style={[
                  {
  ...
  const renderScene = ({ route }) => {
    return <ListProduct route={route} />;
    // switch (route.key) {
    //     case 'customer':
