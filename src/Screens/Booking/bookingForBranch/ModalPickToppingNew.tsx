...
import { Platform, KeyboardAvoidingView, StyleSheet, View, TextInput, ScrollView, Image, TouchableOpacity, FlatList, LayoutAnimation } from 'react-native';
import { stylesFont } from '../../../Constant/Font';
import { _moderateScale, _widthScale, _width } from '../../../Constant/Scale';
import { WHITE, BG_GREY_OPACITY_2, BG_GREY_OPACITY_5, GREY, BLACK_OPACITY_8, BLUE_FB, GREY_FOR_TITLE, PRICE_ORANGE, BLACK_OPACITY_7, BLACK, BASE_COLOR, BLUE, GREEN_SUCCESS, RED } from '../../../Constant/Color';
                                ...
                                props?.data?.isOnFlashSale && props?.data?.preferentialInCurrentFlashSale ? (
                                    <Row gap={_moderateScale(10)}>
                                        <Text style={[stylesFont.fontNolanBold, { fontSize: _moderateScale(16), color: PRICE_ORANGE }]}>
                                            {formatMonney(props?.data?.preferentialInCurrentFlashSale?.finalPrice)}
                    ...
                    }]}>
                    {
                        props?.data?.isOnFlashSale && props?.data?.preferentialInCurrentFlashSale ? (
                            <Text style={[stylesFont.fontNolanBold, { fontSize: _moderateScale(16), color: WHITE }]}>
                                Xác nhận ({
                                    formatMonney(
                                        props?.data?.preferentialInCurrentFlashSale?.finalPrice + props?.listChoice?.reduce((total, item) => {
                                            return total += (item?.extraAmount != null ? item.extraAmount : 0);
                                        }, 0)
                                    )
                                })
                            </Text>
            ...
            <View style={{ flex: 1 }}>
                {
                    props?.currDescriptionTopping?.content ? (
                        ...
                        <Column flex={1} justifyContent='center'>
                            <Text color={GREY} size={_moderateScale(16)} fontStyle='italic' style={{ textAlign: 'center' }}>Chưa có mô tả</Text>
                        </Column>
                    )
                }
...
const ModalPickToppingNew = ({ confirm, data, show, hide }: Props) => {
    ...
    const _handleChoiceSingle = (data, groupCode) => {
        ...
        let indexItem = tempListChoice?.findIndex(item => item?.code == data?.code && item?.groupCode == data?.groupCode);
        if (indexItem !== -1) {
