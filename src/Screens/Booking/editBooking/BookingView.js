...
import { Image, StyleSheet, Text, TouchableOpacity, ScrollView, View, TextInput, ImageBackground } from 'react-native';
...
import { getListBranchLocation, createNewBooking, updateBooking } from '../../../Redux/Action/BookingAction'
...
const BookingView = memo((props) => {
    ...
    useEffect(() => {
        if (props?.dataBooking?._id) {
            if (props?.dataBooking?.referralAncestorArr?.length > 0) {
                setCodeRef(props?.dataBooking?.referralAncestorArr[0]?.code)
            }

            setBranchMain(props?.dataBooking?.branch)
            setBranchForBooking(props?.dataBooking?.branch?.code)

            setListServiceHasChoice(props?.dataBooking?.servicesNeedCare)

            setDescription(props?.dataBooking?.description)

            setCurrPickDate(moment(props?.dataBooking?.appointmentDateFinal?.date).format())
            // props?.dataBooking?.referralAncestorArr?.length > 0 ? props?.dataBooking?.referralAncestorArr[0]?.code: ''
        }
    }, [props?.dataBooking])
    ...
    const _handleConfirmCreateBooking = async () => {
        ...
        let resultCreateNewBooking = await updateBooking(dataFetchCreateBooking,props?.dataBooking?._id);
    ...
    return (
        <>
            ...
            <View style={[styleElement.rowAliCenter, { paddingHorizontal: _moderateScale(8 * 2), flex: 1, marginTop: _moderateScale(8 * 2) }]}>
                <TextInput
                    editable={props?.dataBooking?.isConfirmReferral ? false : true}
            ...
            {
                props?.dataBooking?.assignedDoctorArr?.length > 0 ?
                                ...
                                return (
                                    ...
                                    <View style={[{
                                        ...
                                        <ImageBackground
                                            ...
                                            imageStyle={{ borderWidth: 1, borderRadius: _moderateScale(8), backgroundColor: Color.BG_GREY_OPACITY_2, borderColor:BG_GREY_OPACITY_5}}
                                            source={{
